...
\begin{abstract}
GPU-based logic simulation exposes many fine-grained operations with strict data dependencies. Although gates in the same dependency frontier can be evaluated in parallel, each individual logic operation is extremely lightweight, so host-side scheduling overhead and CUDA kernel launch overhead can dominate total runtime. This project studies the tradeoff between scheduling flexibility and execution granularity for circuit task graphs. We parse benchmark circuits into directed acyclic graphs, implement several gate-level scheduling policies, and compare batch blocking, batch non-blocking, levelization, and fused levelization. Our results show that ready-queue ordering alone has limited impact for very small gate operations, while changing the execution granularity through levelized and fused-level execution provides a more direct performance benefit.
\end{abstract}
\maketitle
