...
void GPUSimulator::_run_gates_DSP_gpu_FIFO(const int _total_num_levels,
  ...
  uint32_t *_pi_gate_po_output_res_gpu) 
{
  ...
  for (size_t rd = 0; rd < _num_rounds; rd++) {
    ...
    for (int level = 0; level < _total_num_levels; level++) {
      ...
      _run_gate_DSP_FIFO <<< num_blocks, num_threads >>> (num_accumGates, _numGates_per_level_gpu, 
...
__global__ void _run_gate_DSP_FIFO(const int num_accumGates, const int *_numGates_per_level_gpu,
